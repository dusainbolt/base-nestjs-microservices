...
async function checkDatabaseConsistency(expected: ExpectedCategory[]) {
  ...
  expected.forEach((ex) => {
    ...
    if (!found) {
      discrepancies.push(
        `MISSING in DB: [${ex.type}${ex.subCategory ? '/' + ex.subCategory : ''}] - ${ex.title}`,
      );
    }
  });
  ...
  dbCategories.forEach((db) => {
    ...
    if (!found) {
      discrepancies.push(
        `EXTRA in DB (Not in JSON): [${db.type}${db.subCategory ? '/' + db.subCategory : ''}] - ${db.name} (Code: ${db.code})`,
      );
    }
  });
    ...
    if (problems.length > 0) {
      ...
      for (const [group, items] of Object.entries(grouped)) {
        ...
        (items as any[]).forEach((item) => {
          console.log(
            `  - Level ${item.Level} | ${item['Category Code']} | ${item.Packs} packs [${item.Status}]`,
          );
        });
      }
    } else {
