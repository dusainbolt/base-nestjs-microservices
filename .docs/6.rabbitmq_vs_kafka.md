...
1. Dùng RabbitMQ khi...Dùng Kafka khi...Bạn cần truyền tin tin cậy, xác nhận từng tin nhắn (Ack).Bạn xử lý hàng triệu sự kiện mỗi giây (Logging, Tracking).Bạn cần các tính năng Exchange/Routing phức tạp.Bạn cần "re-play" (đọc lại) dữ liệu trong quá khứ.Hệ thống của bạn chủ yếu là Request-Response.Bạn đang xây dựng Event Streaming hoặc Data Pipeline.Bạn ưu tiên sự đơn giản và dễ vận hành ban đầu.Bạn cần khả năng mở rộng (Scaling) không giới hạn.
